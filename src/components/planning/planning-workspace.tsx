...
export function PlanningWorkspace({ teamId, teamBudget, players, buckets, plans, initialSelections, initialPlan, canEdit }: { teamId: string; teamBudget: number; players: PlayerWithBucket[]; buckets: AuctionBucket[]; plans: ProbableTeam[]; initialSelections: ProbableSelection[]; initialPlan: PlanLabel; canEdit: boolean }) {
  ...
  const profilePlayer = profilePlayerId ? players.find(player => player.id === profilePlayerId) ?? null : null;

  useEffect(() => {
    if (!profilePlayerId) return;
    function closeOnEscape(event: KeyboardEvent) { if (event.key === "Escape") setProfilePlayerId(null); }
  ...
  async function move(selection: ProbableSelection, direction: "up" | "down") {
    ...
    const visibleIndex = visibleSelected.findIndex(item => item.id === selection.id);
    const targetVisibleIndex = direction === "up" ? visibleIndex - 1 : visibleIndex + 1;
    if (visibleIndex < 0 || targetVisibleIndex < 0 || targetVisibleIndex >= visibleSelected.length) return;
    const targetSelection = visibleSelected[targetVisibleIndex];
    const index = ordered.findIndex(item => item.id === selection.id);
    const target = ordered.findIndex(item => item.id === targetSelection.id);
    ...
    try {
      const result = await movePlanPlayer(teamId, selection.id, direction, targetSelection.id);
      if (!result.ok) { setSelections(previous); setMessage({ kind: "error", text: result.message }); }
      else setMessage({ kind: "success", text: `Player moved ${direction} in Plan ${activePlan}.` });
    } catch {
      setSelections(previous);
  ...
  return <>
    ...
    <div className="mt-5 grid items-start gap-5 xl:grid-cols-[minmax(0,1.05fr)_minmax(24rem,0.95fr)]">
      <section className="order-2 min-w-0 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5 xl:order-1">
        ...
        {shortlistBucketId ? <div className="mt-3 rounded-xl bg-slate-50 px-3 py-3 text-xs font-semibold text-slate-600"><div className="flex items-center justify-between gap-3"><span>Showing {visibleSelected.length} of {selected.length} players</span><button type="button" onClick={() => setShortlistBucketId("")} className="team-accent-text min-h-9 px-2 font-bold">Show all</button></div><p className="mt-1 text-slate-500">Reorder controls move players within this bucket view.</p></div> : null}
        <div className="mt-4 space-y-3">{visibleSelected.map((selection, visibleIndex) => { const index = selected.findIndex(item => item.id === selection.id); const player = players.find(item => item.id === selection.player_id); if (!player) return null; const moving = pendingKeys.has(`move:${selection.id}`), removing = pendingKeys.has(`remove:${selection.id}`); return <article key={selection.id} className={`rounded-xl border border-slate-200 p-3 transition sm:p-4 ${moving ? "team-soft opacity-70" : selection.id.startsWith("optimistic-") ? "team-soft" : "bg-white"}`}><div className="flex flex-col gap-3"><div className="flex items-start gap-3"><span className="team-soft grid h-9 w-9 shrink-0 place-items-center rounded-full border text-sm font-black">{visibleIndex + 1}</span><div className="min-w-0 flex-1"><p className="truncate font-bold text-ink">{player.name}</p><p className="mt-1 text-sm text-slate-500">{player.role ?? "Role not set"} · {player.auction_buckets?.name ?? "Unassigned"}</p><p className="mt-1 text-xs sm:text-sm">{player.priority ? PRIORITY_LABELS[player.priority] : "Not ranked"} · {AVAILABILITY_LABELS[player.availability_status]} · Expected {formatMoney(player.expected_price)}</p>{shortlistBucketId ? <p className="mt-1 text-[11px] font-semibold text-slate-400">Overall plan position {index + 1}</p> : null}</div></div><div className="flex flex-wrap items-center justify-between gap-2 border-t pt-2"><button type="button" onClick={() => setProfilePlayerId(player.id)} className="team-accent-text min-h-10 px-2 text-sm font-bold underline-offset-4 hover:underline">View profile</button>{canEdit ? <div className="flex items-center gap-1"><button title="Move player up" disabled={visibleIndex === 0 || moving || removing || selection.id.startsWith("optimistic-")} onClick={() => move(selection, "up")} aria-label={`Move ${player.name} up`} className="min-h-10 rounded-lg border px-3 text-xs font-bold hover:bg-slate-50 disabled:opacity-30">↑ Up</button><button title="Move player down" disabled={visibleIndex === visibleSelected.length - 1 || moving || removing || selection.id.startsWith("optimistic-")} onClick={() => move(selection, "down")} aria-label={`Move ${player.name} down`} className="min-h-10 rounded-lg border px-3 text-xs font-bold hover:bg-slate-50 disabled:opacity-30">↓ Down</button><button disabled={moving || removing || selection.id.startsWith("optimistic-")} onClick={() => remove(selection)} className="min-h-10 rounded-lg px-3 text-xs font-bold text-red-700 hover:bg-red-50 disabled:opacity-50">{removing ? "Removing..." : "Remove"}</button></div> : null}</div></div></article>; })}
    ...
    {profilePlayer ? <PlayerProfileModal player={profilePlayer} onClose={() => setProfilePlayerId(null)} /> : null}
  </>;
}

function PlayerProfileModal({ player, onClose }: { player: PlayerWithBucket; onClose: () => void }) {
  return <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-950/55 p-0 sm:items-center sm:p-5" onMouseDown={event => { if (event.target === event.currentTarget) onClose(); }}>
    <section role="dialog" aria-modal="true" aria-labelledby="planning-player-profile-title" className="max-h-[92dvh] w-full overflow-y-auto rounded-t-3xl bg-white shadow-2xl sm:max-w-2xl sm:rounded-3xl">
      <header className="sticky top-0 z-10 flex items-start justify-between gap-4 border-b bg-white p-4 sm:p-6"><div><p className="team-accent-text text-xs font-bold uppercase">Player profile</p><h2 id="planning-player-profile-title" className="mt-1 text-2xl font-black">{player.name}</h2><p className="mt-1 text-sm text-slate-500">{player.role ?? "Role not set"} · {player.auction_buckets?.name ?? "Unassigned"}</p></div><button type="button" autoFocus onClick={onClose} aria-label="Close player profile" className="min-h-11 rounded-xl border bg-white px-4 text-sm font-bold text-slate-700 hover:bg-slate-50">Close</button></header>
      <div className="p-4 sm:p-6">
        <dl className="grid grid-cols-2 gap-4 sm:grid-cols-3"><ProfileItem label="Priority" value={player.priority ? PRIORITY_LABELS[player.priority] : "Not ranked"} /><ProfileItem label="Availability" value={AVAILABILITY_LABELS[player.availability_status]} /><ProfileItem label="Expected price" value={formatMoney(player.expected_price)} /><ProfileItem label="Available matches" value={player.available_matches === null ? "Not specified" : String(player.available_matches)} /><ProfileItem label="Bucket" value={player.auction_buckets?.name ?? "Unassigned"} /><ProfileItem label="Role" value={player.role ?? "Not set"} /></dl>
        <div className="mt-6"><h3 className="text-sm font-black">Career / Imported Stats</h3><dl className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4"><ProfileStat label="Matches" value={player.matches} /><ProfileStat label="Batting" value={player.batting_score} /><ProfileStat label="Wickets" value={player.bowling_wickets} /><ProfileStat label="Catches" value={player.catches} /></dl></div>
        <div className="mt-6 grid gap-4 sm:grid-cols-2"><ProfileNotes label="Availability notes" value={player.availability_notes} /><ProfileNotes label="Player notes" value={player.notes} /></div>
      </div>
      <footer className="sticky bottom-0 border-t bg-white p-4 sm:px-6"><button type="button" onClick={onClose} className="team-primary min-h-12 w-full rounded-xl px-5 text-sm font-bold sm:ml-auto sm:block sm:w-auto">Close profile</button></footer>
    </section>
  </div>;
}

function ProfileItem({ label, value }: { label: string; value: string }) { return <div><dt className="text-xs font-bold uppercase text-slate-400">{label}</dt><dd className="mt-1 font-semibold text-slate-700">{value}</dd></div>; }
function ProfileStat({ label, value }: { label: string; value: number }) { return <div className="rounded-xl bg-slate-50 p-3 text-center"><dt className="text-[10px] font-bold uppercase text-slate-500">{label}</dt><dd className="mt-1 text-xl font-black">{value}</dd></div>; }
function ProfileNotes({ label, value }: { label: string; value: string | null }) { return <div className="rounded-xl border p-4"><h3 className="text-xs font-bold uppercase text-slate-400">{label}</h3><p className="mt-2 whitespace-pre-wrap text-sm leading-6 text-slate-600">{value || "No notes added."}</p></div>; }
