...
export default async function PlayersPage({ params, searchParams }: { params: Promise<{ teamId: string }>; searchParams: Promise<Record<string, string | undefined>> }) {
  ...
  const [{ canEdit }, players, buckets] = await Promise.all([requireTeamAccess(teamId), getPlayers(teamId), getBuckets(teamId)]);
  ...
  return <div className="mx-auto max-w-7xl px-4 py-7 sm:px-6 lg:px-8">
    ...
    <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-sm font-bold text-pitch">Private team strategy</p><h1 className="mt-1 text-3xl font-bold text-ink">Player pool</h1><p className="mt-2 text-sm text-slate-500">Search, rank and organise auction targets.</p></div>{canEdit ? <div className="flex gap-2"><Link href={`/teams/${teamId}/players/import`} className="min-h-11 rounded-xl border border-pitch px-4 py-3 text-sm font-bold text-pitch">Import players</Link><Link href={`/teams/${teamId}/players/new`} className="min-h-11 rounded-xl bg-pitch px-4 py-3 text-sm font-bold text-white">Add player</Link></div> : null}</div>
    ...
    <div className="mt-3 hidden overflow-x-auto rounded-2xl border border-slate-200 bg-white md:block"><table className="w-full text-left text-sm"><thead className="bg-slate-50 text-xs uppercase tracking-wide text-slate-500"><tr>{["Player","Role","Bucket","Priority","Availability","Expected","Status",""].map(v => <th key={v} className="px-4 py-3">{v}</th>)}</tr></thead><tbody>{shown.map(player => <tr key={player.id} className="border-t border-slate-100"><td className="px-4 py-4 font-bold text-ink">{player.name}</td><td className="px-4 py-4">{player.role ?? "—"}</td><td className="px-4 py-4">{player.auction_buckets?.name ?? "Unassigned"}</td><td className="px-4 py-4">{player.priority ? PRIORITY_LABELS[player.priority] : "Not ranked"}</td><td className="px-4 py-4">{AVAILABILITY_LABELS[player.availability_status]}</td><td className="px-4 py-4">{formatMoney(player.expected_price)}</td><td className="px-4 py-4">{AUCTION_STATUS_LABELS[player.auction_status]}</td><td className="px-4 py-4">{canEdit ? <Link href={`/teams/${teamId}/players/${player.id}/edit`} className="font-bold text-pitch">Edit</Link> : null}</td></tr>)}</tbody></table></div>
    <div className="mt-3 grid gap-3 md:hidden">{shown.map(player => <article key={player.id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex justify-between gap-3"><div><h2 className="text-lg font-bold text-ink">{player.name}</h2><p className="mt-1 text-sm text-slate-500">{player.role ?? "Role not set"}</p></div>{canEdit ? <Link href={`/teams/${teamId}/players/${player.id}/edit`} className="min-h-11 px-2 py-3 text-sm font-bold text-pitch">Edit</Link> : null}</div><dl className="mt-4 grid grid-cols-2 gap-3 text-sm"><Item label="Priority" value={player.priority ? PRIORITY_LABELS[player.priority] : "Not ranked"}/><Item label="Bucket" value={player.auction_buckets?.name ?? "Unassigned"}/><Item label="Availability" value={AVAILABILITY_LABELS[player.availability_status]}/><Item label="Expected" value={formatMoney(player.expected_price)}/></dl></article>)}</div>
