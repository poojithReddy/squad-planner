...
export async function changeAuctionStatus(teamId:string,playerId:string,expectedStatus:AuctionStatus,newStatus:AuctionStatus,soldPrice=0,overrideSquad=false,overrideBucket=false):Promise<AuctionMutationResult>{
  ...
  const {data:team}=await supabase.from("teams").select("auction_status").eq("id",teamId).maybeSingle();
  if(team?.auction_status==="completed")return{ok:false,code:"forbidden",message:"Reopen the auction before changing player decisions."};
  const{data,error}=await supabase.rpc("update_player_auction_status",{p_team_id:teamId,p_player_id:playerId,p_expected_status:expectedStatus,p_new_status:newStatus,p_sold_price:soldPrice,p_override_squad_limit:overrideSquad,p_override_bucket_max:overrideBucket});
