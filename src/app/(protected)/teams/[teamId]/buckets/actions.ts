...
export async function createBucket(teamId: string, _state: FormState, formData: FormData): Promise<FormState> { await requireTeamAccess(teamId, true); const parsed = parse(formData); if (!parsed.data) return { status: "error", message: parsed.error ?? "Invalid bucket." }; const supabase = await createClient(); const { data: last } = await supabase.from("auction_buckets").select("display_order").eq("team_id", teamId).order("display_order", { ascending: false }).limit(1).maybeSingle(); const { error } = await supabase.from("auction_buckets").insert({ team_id: teamId, display_order: (last?.display_order ?? -1) + 1, ...parsed.data }); if (error) return { status: "error", message: "We couldn't create this bucket. Please try again." }; revalidatePath(`/teams/${teamId}/buckets`); return { status: "success", message: "Bucket created." }; }
